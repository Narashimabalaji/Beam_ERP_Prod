...
import { Plus, TrendingUp, Calendar, Pencil, Trash2, X, Check, IndianRupee, ArrowUpRight, BarChart3, Filter } from 'lucide-react';

const MONTHS = ['All', 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];

export default function Revenue() {
  const [revenue, setRevenue] = useState([]);
  const [stats, setStats] = useState({ total_revenue: 0, entry_count: 0, average_transaction: 0 });
  ...
  const fetchAllData = async () => {
    setLoading(true);
    try {
      let url = `http://localhost:8000/api/stats/revenue?year=${filterYear}`;
      if (filterMonth > 0) url += `&month=${filterMonth}`;
      if (filterDay > 0) url += `&day=${filterDay}`;
      
      const res = await axios.get(url);
      setRevenue(res.data.records);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`http://localhost:8000/revenue/${editingId}`, payload);
        setEditingId(null);
      } else {
        await axios.post('http://localhost:8000/revenue', payload);
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:8000/revenue/${id}`);
      fetchAllData();
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-brand-900 tracking-tight">Revenue Management</h2>
          <p className="text-slate-500 text-sm mt-1">Track and analyze all incoming revenue streams</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-brand-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Filtered Revenue</p>
            <p className="text-2xl font-black text-brand-900">{fmt(stats.total_revenue)}</p>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-brand-100 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-black text-brand-900">{stats.entry_count}</p>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-brand-100 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-black text-brand-900">{fmt(stats.average_transaction)}</p>
          </div>
        </div>
      </div>
      ...
      {showForm && (
        <div className="bg-gradient-to-br from-white to-brand-50 p-6 rounded-2xl shadow-md border border-brand-200 relative overflow-hidden">
          ...
          <h3 className="text-lg font-bold text-brand-900 mb-5 flex items-center gap-2">
            ...
            {editingId ? 'Edit Revenue Entry' : 'Record New Income'}
          </h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-5 relative z-10">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div>
                ...
                <input 
                  type="text" required value={formData.source}
            ...
            <div>
              ...
              <textarea 
                ...
                className="w-full px-4 py-3 rounded-xl border border-brand-200 focus:outline-none focus:ring-2 focus:ring-brand-500 bg-white shadow-sm"
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-brand-100 overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="text-center py-16 flex flex-col items-center">
            ...
            <h4 className="text-lg font-bold text-brand-900">No Revenue Found</h4>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-white border-b border-brand-100 text-slate-400 text-xs uppercase tracking-wider font-bold">
                  <th className="px-5 py-4 w-[25%]">Source</th>
                  ...
                  <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors group">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-bold text-brand-900 truncate block">{item.source}</span>
                      </div>
                    </td>
