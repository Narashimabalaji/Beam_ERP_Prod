...
function makeInvoice() {
  let savedSettings = {};
  try {
    const saved = localStorage.getItem('beam-erp-invoice');
...
export default function Invoices() {
  ...
  const [invoice, setInvoice] = useState(() => {
    const saved = localStorage.getItem('beam-erp-invoice');
    if (saved) {
      try { return JSON.parse(saved); } catch (e) { return makeInvoice(); }
    }
    return makeInvoice();
  });
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      localStorage.setItem('beam-erp-invoice', JSON.stringify(invoice));
    }, 400);
    return () => clearTimeout(timer);
  }, [invoice]);

  const subtotal = useMemo(
    () => invoice.items.reduce((sum, item) => sum + (Number(item.qty || 1) * Number(item.rate || 0)), 0),
    [invoice.items],
  );
  const gst = useMemo(
  ...
  const addItem = () => setInvoice((cur) => ({ ...cur, items: [...cur.items, { ...emptyItem }] }));

  const removeItem = (index) => {
    if (invoice.items.length === 1) return;
    setInvoice((cur) => ({ ...cur, items: cur.items.filter((_, i) => i !== index) }));
  };

  const clearItems = () => {
    if (window.confirm('Clear all items?')) {
      setInvoice((cur) => ({ ...cur, items: [{ ...emptyItem }] }));
    }
  };

  const resetForm = () => {
    if (window.confirm('Reset entire invoice form?')) {
  ...
  const handlePreview = async (e) => {
    e.preventDefault();
    try {
      const finalInvoice = { ...invoice, subtotal, gst, total };
      const url = await generateInvoicePDF(finalInvoice, 'blob', 'TAX INVOICE');
      setPreviewUrl(url);
    } catch (err) {
      console.error(err);
  ...
  const handleDownload = async () => {
    try {
      const finalInvoice = { ...invoice, subtotal, gst, total };
      await generateInvoicePDF(finalInvoice, 'save', 'TAX INVOICE');
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-brand-900 tracking-tight">Invoice Generator</h2>
          <p className="text-slate-500 text-sm mt-1">Generate professional PDF invoices with automatic GST math</p>
        </div>
      </div>
      ...
      {previewUrl ? (
        <div className="space-y-4 animate-fade-in">
          ...
          <iframe src={previewUrl} title="Invoice Preview" className="w-full h-[80vh] rounded-2xl border border-brand-200 shadow-sm" />
        </div>
      ) : (
        <form onSubmit={handlePreview} className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-brand-100 shadow-sm overflow-hidden">
            ...
            <div className="p-5 grid grid-cols-1 md:grid-cols-4 gap-5">
              <div className="md:col-span-2">
                ...
                <input name="fromName" value={invoice.fromName} onChange={update} maxLength={50} className="w-full px-4 py-2 rounded-xl border border-brand-200 text-slate-900 bg-white focus:ring-2 focus:ring-brand-500 font-bold" />
              </div>
              <div className="md:col-span-2">
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">GSTIN</label>
                <input name="fromGstin" value={invoice.fromGstin} onChange={update} maxLength={15} className="w-full px-4 py-2 rounded-xl border border-brand-200 text-slate-900 bg-white focus:ring-2 focus:ring-brand-500" />
              </div>
              <div className="md:col-span-2">
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Address</label>
                <textarea name="fromAddress" value={invoice.fromAddress} onChange={update} rows={2} maxLength={150} className="w-full px-4 py-2 rounded-xl border border-brand-200 text-slate-900 bg-white focus:ring-2 focus:ring-brand-500" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Email</label>
                <input name="fromEmail" value={invoice.fromEmail} onChange={update} maxLength={50} className="w-full px-4 py-2 rounded-xl border border-brand-200 text-slate-900 bg-white focus:ring-2 focus:ring-brand-500" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Phone Number</label>
                <input name="fromPhone" value={invoice.fromPhone} onChange={update} maxLength={15} className="w-full px-4 py-2 rounded-xl border border-brand-200 text-slate-900 bg-white focus:ring-2 focus:ring-brand-500" />
              </div>
              <div className="md:col-span-2">
                ...
                <div className="flex items-center gap-4">
                  ...
                  {invoice.fromLogo && (
                    <div className="relative">
                      <img src={invoice.fromLogo} alt="Logo preview" className="h-10 w-auto object-contain rounded border border-slate-200" />
              ...
              <div className="md:col-span-2">
                ...
                <div className="flex items-center gap-4">
                  ...
                  {invoice.fromSignature && (
                    <div className="relative">
                      <img src={invoice.fromSignature} alt="Signature preview" className="h-10 w-auto object-contain rounded border border-slate-200" />
          ...
          <div className="bg-white rounded-2xl border border-brand-100 shadow-sm overflow-hidden">
            ...
            <div className="p-5 grid grid-cols-1 md:grid-cols-4 gap-5">
              <div>
                ...
                <input required name="invoiceNo" value={invoice.invoiceNo} onChange={update} maxLength={20} className="w-full px-4 py-2 rounded-xl border border-brand-200 text-slate-900 bg-white focus:ring-2 focus:ring-brand-500 font-bold" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Invoice Date</label>
                <input required type="date" name="date" value={invoice.date} onChange={update} className="w-full px-4 py-2 rounded-xl border border-brand-200 text-slate-900 bg-white focus:ring-2 focus:ring-brand-500" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Due Date</label>
                <input type="date" name="dueDate" value={invoice.dueDate} onChange={update} className="w-full px-4 py-2 rounded-xl border border-brand-200 text-slate-900 bg-white focus:ring-2 focus:ring-brand-500" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Place of Supply</label>
                <input name="placeOfSupply" value={invoice.placeOfSupply} onChange={update} maxLength={30} className="w-full px-4 py-2 rounded-xl border border-brand-200 text-slate-900 bg-white focus:ring-2 focus:ring-brand-500" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-brand-100 shadow-sm overflow-hidden">
            ...
            <div className="p-5 grid grid-cols-1 md:grid-cols-3 gap-5">
              <div className="md:col-span-2">
                ...
                <input required name="clientName" value={invoice.clientName} onChange={update} maxLength={50} className="w-full px-4 py-2 rounded-xl border border-brand-200 text-slate-900 bg-white focus:ring-2 focus:ring-brand-500" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Phone</label>
                <input name="phone" value={invoice.phone} onChange={update} maxLength={15} className="w-full px-4 py-2 rounded-xl border border-brand-200 text-slate-900 bg-white focus:ring-2 focus:ring-brand-500" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Email Address</label>
                <input name="email" type="email" value={invoice.email} onChange={update} maxLength={50} className="w-full px-4 py-2 rounded-xl border border-brand-200 text-slate-900 bg-white focus:ring-2 focus:ring-brand-500" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">GSTIN</label>
                <input name="gstin" value={invoice.gstin} onChange={update} maxLength={15} className="w-full px-4 py-2 rounded-xl border border-brand-200 text-slate-900 bg-white focus:ring-2 focus:ring-brand-500" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">PAN</label>
                <input name="pan" value={invoice.pan} onChange={update} maxLength={10} className="w-full px-4 py-2 rounded-xl border border-brand-200 text-slate-900 bg-white focus:ring-2 focus:ring-brand-500" />
              </div>
              <div className="md:col-span-3">
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Billing Address</label>
                <textarea name="billingAddress" value={invoice.billingAddress} onChange={update} rows={2} maxLength={150} className="w-full px-4 py-2 rounded-xl border border-brand-200 text-slate-900 bg-white focus:ring-2 focus:ring-brand-500" />
              </div>
            </div>
          </div>
              ...
              {invoice.items.map((item, index) => (
                <div key={index} className="grid grid-cols-1 md:grid-cols-5 gap-4 p-4 border border-brand-100 bg-slate-50/50 rounded-xl relative">
                  ...
                  <div className="md:col-span-6">
                    ...
                    <input list={`materials-list-${index}`} required value={item.title} onChange={(e) => updateItem(index, 'title', e.target.value)} className="w-full px-3 py-2 rounded-lg border border-brand-200 text-slate-900 bg-white font-bold text-brand-900" />
                  ...
                  <button type="button" onClick={() => removeItem(index)} disabled={invoice.items.length === 1} className="absolute -top-3 -right-3 bg-white border border-red-200 text-red-500 rounded-full p-1.5 shadow-sm hover:bg-red-50 disabled:opacity-50">
                    <Trash2 size={14} />
                  </button>
                </div>
              ))}
          ...
          <div className="bg-white rounded-2xl border border-brand-100 shadow-sm overflow-hidden p-5">
            ...
            <textarea name="termsConditions" value={invoice.termsConditions} onChange={update} rows={3} maxLength={500} className="w-full px-4 py-2 rounded-xl border border-brand-200 text-slate-900 bg-white focus:ring-2 focus:ring-brand-500" placeholder="Enter your terms and conditions here..." />
          </div>
          ...
          <div className="bg-slate-900 rounded-2xl shadow-xl p-6 text-white grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            ...
            <div className="flex gap-8">
              <div>
                ...
                <input value={invoice.gstPercent} onChange={(e) => setInvoice(cur => ({...cur, gstPercent: e.target.value}))} className="w-20 px-3 py-2 rounded-lg bg-slate-800 border border-slate-700 text-white font-bold" />
              </div>
